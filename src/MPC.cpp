 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
	...
	for(size_t t = 0 ;t < N - 1; t++){
		...
		fg[0] += CppAD::pow(vars[v_start+t]*vars[v_start+t]/r_curve,2);	
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  std::cout << "Cost:" << cost <<"\t CTE:"<<solution.x[cte_start]<<"\t EPSI:"<<solution.x[epsi_start];
  std::cout <<"\t Delta:"<<solution.x[delta_start]*180/3.14<<"\t a:"<<solution.x[a_start] <<std::endl;
